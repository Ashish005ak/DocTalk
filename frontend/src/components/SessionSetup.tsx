    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#1a1f2e] border border-[#2d3555] rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {chatModeEnabled && domain === 'medical' && (
            <div>
              ...
              <div className="flex gap-2">
                {([{ value: 'simulation', label: 'Simulation' }, { value: 'chat', label: 'Live Chat' }] as const).map(
                  (opt) => (
                    <button
                      key={opt.value}
          ...
          <button
            onClick={handleStart}
            disabled={(!isChatMode && !selectedTranscript) || loading}
          ...
          >
            {loading ? 'Starting...' : isChatMode ? 'Start Chat' : 'Start Session'}
          </button>
        </div>
      </div>
    </div>
